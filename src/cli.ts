...
function parseArgs(argv: string[]): Options {
  ...
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--provider') provider = argv[++i] ?? '';
      ...
      console.error('Usage: llmchat [--provider gemini] [--login] "prompt"');
      process.exit(0);
...
async function main(): Promise<void> {
  ...
  if (options.provider !== 'gemini')
    throw new Error(`Provider not implemented: ${options.provider}`);
  ...
  try {
    ...
    if (options.login) {
      console.error('[gemini] complete login in the browser, then press Enter here');
