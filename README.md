...
On a failure, `lastError` is a concise human summary and remains compatible with legacy state files. `lastErrorVerbose` retains the complete redacted diagnostic. `npm run loop -- --status` hides that verbose field; `npm run loop -- --status --verbose` includes it for diagnosis.
