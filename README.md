...
Environment overrides and fallback homes must resolve to non-empty absolute paths; storage fails closed for empty, whitespace-only, or relative roots. Backup exclusion is also fail-closed. macOS uses `tmutil addexclusion` followed by `tmutil isexcluded` and accepts only an `[Excluded]` result for the exact canonical root requested. This phase has no secure universal user-level mechanism for Windows backup or the diverse Linux backup tools, so those platforms return an explicit unsupported-platform diagnostic instead of claiming exclusion. The boundary remains injectable for a future verified native strategy. Local development overrides use `.llmchat-data/`, which is ignored by Git; normal data roots are outside the repository.
...
| Local disclosure of sessions, prompts, or responses   | Absolute per-user data root, repaired `0700`/`0600` POSIX modes, verified current-user Windows ACL, Git ignore, and fail-closed native backup boundary. |
